...
function PaymentModal({ show, onClose, totalAmount, onPaymentSuccess, storeConfig }) {
  ...
  const totalPaidCents = payments.reduce((acc, p) => acc + money.toCents(p.amount), 0);
  ...
  // --- LÓGICA: AGREGAR PAGO AUTOMÁTICAMENTE ---
  const handleMethodSelect = (methodId) => {
    ...
    if (!methodConfig.allowsOverpayment) {
      ...
      if (amountCents > currentPendingCents) {
        toast.error(`Máximo: ${currencySymbol} ${money.fromCents(currentPendingCents).toFixed(2)}`);
        setCurrentAmount(money.fromCents(currentPendingCents).toFixed(2));
        setTimeout(() => {
          if (inputRef.current) {
            inputRef.current.focus();
            inputRef.current.select();
    ...
    const newTotalPaidCents = newPaymentsList.reduce((acc, p) => acc + money.toCents(p.amount), 0);
    const remainingCents = Math.max(0, totalAmountCents - newTotalPaidCents);
    
    const newAmount = remainingCents > 0 ? money.fromCents(remainingCents).toFixed(2) : '';
    setCurrentAmount(newAmount);
    
    if (remainingCents > 0) {
      const currentIndex = activeMethods.findIndex(m => m.id === methodId);
      const nextMethod = activeMethods[currentIndex + 1] || activeMethods[0];
  ...
  const removePayment = (index) => {
    ...
    const newPaid = newPayments.reduce((acc, p) => acc + money.toCents(p.amount), 0);
  ...
  const handleConfirm = async () => {
    ...
    if (finalPayments.length === 0 && currentInputCents >= totalAmountCents) {
      const currentMethod = activeMethods.find(m => m.id === selectedMethodId) || activeMethods[0];
      if (currentMethod) {
        finalPayments.push({ 
    ...
    const finalTotalPaidCents = finalPayments.reduce((acc, p) => acc + money.toCents(p.amount), 0);

    if (finalTotalPaidCents < totalAmountCents) {
      const missing = money.fromCents(totalAmountCents - finalTotalPaidCents);
      toast.error(`Falta cubrir: ${currencySymbol} ${missing.toFixed(2)}`);
      return;
    }
    ...
    >
      <div className="bg-white w-full max-w-5xl rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row h-[620px] max-h-[90vh]">
        ...
        <div className="w-full md:w-[340px] bg-gradient-to-br from-slate-50 to-white p-6 flex flex-col border-r border-slate-100">
          ...
          {/* Total - Compacto */}
          <div className="mb-6">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Total a Pagar</p>
            <div className="flex items-baseline gap-1.5">
              <span className="text-lg text-slate-400 font-semibold">{currencySymbol}</span>
              <span className="text-5xl font-black text-slate-900 tracking-tight leading-none">{totalAmount.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="flex-1 mb-5 overflow-hidden flex flex-col min-h-0">
            <div className="flex justify-between items-center mb-2.5">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Pagos</p>
              <span className="text-[10px] font-bold text-slate-400 bg-slate-100 px-1.5 py-0.5 rounded-full">{payments.length}</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 pr-0.5">
              {payments.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-slate-300 py-8">
                  ...
                  <p className="text-[10px] mt-0.5 text-center">Click en método</p>
                </div>
              ) : (
                payments.map((p, index) => (
                  <div key={p.id} className="group flex justify-between items-center p-2.5 bg-white rounded-xl border border-slate-100 hover:border-red-200 hover:shadow-sm transition-all">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="font-bold text-slate-800 text-xs leading-tight">{p.method}</p>
                        <p className="text-[9px] text-slate-400 font-semibold uppercase tracking-wide">{p.type}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-900 text-sm">{currencySymbol} {p.amount.toFixed(2)}</span>
                      <button 
          ...
          <div className={classNames(
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <span className={classNames("text-[10px] font-bold uppercase tracking-wide", isCovered ? "text-emerald-700" : "text-amber-700")}>
                  {isCovered ? "Vuelto" : "Pendiente"}
                </span>
              </div>
              <div className="text-right">
                <span className={classNames("text-2xl font-black", isCovered ? "text-emerald-600" : "text-amber-600")}>
                  {isCovered ? change.toFixed(2) : money.fromCents(pendingCents).toFixed(2)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2">Entrega</p>
        ...
        <div className="flex-1 p-6 flex flex-col bg-white relative">
          ...
          <div className="mb-5">
            ...
            <p className="text-xs text-slate-400 font-medium">
              Click • <kbd className="px-1.5 py-0.5 bg-slate-100 rounded text-[10px] font-mono">F1-F5</kbd> shortcuts • <kbd className="px-1.5 py-0.5 bg-slate-100 rounded text-[10px] font-mono">F12</kbd> finalizar
            </p>
          </div>
            ...
            <div className="bg-gradient-to-br from-slate-50 via-white to-slate-50 rounded-2xl p-5 border border-slate-100 shadow-inner">
              ...
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="text-xs font-bold text-blue-600 bg-blue-50 px-2.5 py-1 rounded-lg">
                  {activeMethods.find(m => m.id === selectedMethodId)?.label || 'Método'}
                </span>
              </div>
            ...
            <span className="relative z-10 flex items-center gap-2.5">
              {isProcessing ? (
                ...
                <>
                  ...
                  <kbd className="bg-white/20 px-2.5 py-1 rounded-lg text-sm font-mono font-bold">F12</kbd>
                </>
              )}
            </span>
          </button>
