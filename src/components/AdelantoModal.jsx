// fuente/components/AdelantoModal.jsx
import React, { useState, useEffect, useRef } from 'react';
import useHotkeys from '../hooks/useHotkeys';
import toast from 'react-hot-toast';

// Este modal recibe el TOTAL del pedido y devuelve el adelanto
function AdelantoModal({ show, onClose, totalAmount, onConfirmAdelanto }) {
    ...
    <div style={styles.modalOverlay}>
      <div style={styles.modalContent}>
        ...
        <div style={styles.totalDisplay}>
          ...
          <span style={styles.totalAmount}>S/ {totalAmount.toFixed(2)}</span>
        </div>
        ...
        <label style={styles.label}>Método de Pago del Adelanto (Defecto: Yape):</label>
        <div style={styles.methodGrid}>
          <button 
            style={paymentMethod === 'efectivo' ? {...styles.methodButton, ...styles.methodButtonSelected} : styles.methodButton}
            onClick={() => setPaymentMethod('efectivo')}>
            <span style={styles.hotkeyLabel}>F1</span>Efectivo
          </button>
          <button 
            style={paymentMethod === 'yape' ? {...styles.methodButton, ...styles.methodButtonSelected} : styles.methodButton}
            onClick={() => setPaymentMethod('yape')}>
            <span style={styles.hotkeyLabel}>F2</span>Yape
          </button>
          <button 
            style={paymentMethod === 'tarjeta' ? {...styles.methodButton, ...styles.methodButtonSelected} : styles.methodButton}
            onClick={() => setPaymentMethod('tarjeta')}>
            <span style={styles.hotkeyLabel}>F3</span>Tarjeta
          </button>
          <button 
            style={paymentMethod === 'banco' ? {...styles.methodButton, ...styles.methodButtonSelected} : styles.methodButton}
        ...
        <div style={styles.buttonGroup}>
          ...
          >
            Confirmar Adelanto <span style={styles.hotkeyLabelSmall}>(F12)</span>
          </button>
        </div>
      </div>
    </div>
...
const styles = {
  ...
  modalContent: { backgroundColor: 'white', padding: '25px 30px', borderRadius: '8px', boxShadow: '0 5px 20px rgba(0,0,0,0.25)', width: '450px', },
  title: { textAlign: 'center', margin: '0 0 15px 0', color: '#1E2A3A' },
  totalDisplay: { backgroundColor: '#f0f2f5', padding: '15px 20px', borderRadius: '5px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' },
  totalText: { fontSize: '18px', fontWeight: 'bold', color: '#555' },
  totalAmount: { fontSize: '28px', fontWeight: 'bold', color: '#1E2A3A' },
  ...
  input: { padding: '12px', fontSize: '18px', borderRadius: '4px', border: '1px solid #ccc', marginTop: '5px', textAlign: 'center', fontWeight: 'bold' },
  methodGrid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' },
  methodButton: { padding: '15px', fontSize: '16px', cursor: 'pointer', border: '2px solid #ddd', borderRadius: '5px', backgroundColor: '#f9f9f9', transition: 'all 0.2s', textAlign: 'center' },
  methodButtonSelected: { backgroundColor: '#007bff', color: 'white', borderColor: '#007bff', fontWeight: 'bold' },
  ...
  hotkeyLabel: { display: 'block', fontSize: '12px', fontWeight: 'bold', color: 'inherit', marginBottom: '5px' },
