// src/features/pos/components/interface/PosInterface.jsx
...
function PosInterface({ cart, onCartChange, onSaleComplete, isActive, storeConfig }) {
  ...
  const returnFocusToSearch = () => {
    if (searchInputRef.current && isShiftOpen) {
  ...
  const removeFromCart = (productId) => {
    if (checkLock()) return;
    onCartChange(safeCart.filter((item) => item.id !== productId));
  };
  ...
  const handleDiscountChange = (itemId, value, type) => {
    ...
    if (totalItemPrice === 0) {
      ...
      discount = totalItemPrice * (discountPercent / 100);
    } else {
      discount = parseFloat(value) || 0;
      if (discount > totalItemPrice) {
        discount = totalItemPrice;
        toast.error('Excede el total.');
      }
      discountPercent = (discount / totalItemPrice) * 100;
    }
  ...
  const updateQuantity = (productId, newQuantity) => {
    ...
    if (
      productInCart.id &&
      !productInCart.id.startsWith('RAPIDO-') &&
      qty > productInCart.stock
    ) {
  ...
  const handleCloseDailySales = () => {
    setIsDailySalesOpen(false);
    returnFocusToSearch();
  };
  const handleCloseOpenRegister = () => {
    setIsOpenRegisterOpen(false);
    returnFocusToSearch();
  };
  const handleCloseCloseRegister = () => {
    setIsCloseRegisterOpen(false);
    returnFocusToSearch();
  };
  ...
  const handleSelectProductFromDept = (product) => {
    addToCart(product);
    ...
    <div className="flex h-full w-full bg-slate-50 font-sans">
                ...
                >
                  ...
                  <div className="flex-1 flex justify-center">
                    {isEditingPrice ? (
                      <input
                        ...
                        autoFocus
                        onBlur={(e) =>
                          handlePriceEdit(item.id, e.target.value)
                        }
                        onKeyDown={(e) => {
                      ...
                      <div
                        ...
                        className="cursor-pointer flex items-center gap-1 group/price"
                      >
                        ...
                        <Edit2
                          size={10}
                          className="text-blue-400 opacity-0 group-hover/price:opacity-100 transition-opacity"
                        />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-[1.4] flex items-center justify-center gap-1">
                    <button
                      ...
                      className="w-6 h-6 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 hover:bg-red-100 hover:text-red-600 transition-colors"
                    >
                      <Minus size={12} />
                    </button>
                    <input
                    ...
                    <button
                      ...
                      className="w-6 h-6 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 hover:bg-green-100 hover:text-green-600 transition-colors"
                    >
                      <Plus size={12} />
                    </button>
                  </div>
      ...
      <div className="w-80 bg-slate-50 border-l border-slate-200 flex flex-col shadow-lg z-10">
        ...
        <div className="p-4 grid grid-cols-3 gap-2 border-b border-slate-200 bg-white">
          ...
          <button
            onClick={() => {
              if (!checkLock()) setIsExpenseModalOpen(true);
            }}
          ...
          <button
            ...
            className="col-span-3 flex items-center justify-center gap-2 py-2 mt-1 rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 text-xs font-bold uppercase transition-colors"
          >
          ...
          <div className="space-y-2 mb-4 text-sm text-slate-600">
            ...
            {itemDiscounts > 0 && (
              <div className="flex justify-between text-red-500">
                ...
                <span>
                  - {currencySymbol} {itemDiscounts.toFixed(2)}
                </span>
              </div>
            )}
            ...
            <div className="flex justify-between items-center group">
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={() => {
                    if (!checkLock()) setIsDiscountModalOpen(true);
                  }}
      ...
      <GlobalDiscountModal
        ...
        currentTotal={subtotal - itemDiscounts}
      ...
      <OpenRegisterModal
        ...
        onOpenShift={handleOpenRegister}
        isOpen={isShiftOpen}
      />
      <CloseRegisterModal
        show={isCloseRegisterOpen}
        onClose={() => setIsCloseRegisterOpen(false)}
        onCloseShift={handleCloseRegister}
